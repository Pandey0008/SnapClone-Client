...
const ChatThread = () => {
  ...
  const { messages, conversations, typingUsers } = useAppSelector(
    (state) => state.chat,
  );
    ...
    const loadMessages = async () => {
      try {
        ...
        const response = await fetch(
          `${API_BASE_URL}/api/v1/chat/messages/${roomId}`,
          {
            method: "GET",
            headers: {
        ...
        if (response.ok) {
          ...
          dispatch(
            setMessagesByRoom({ roomId, messages: data.messages || [] }),
          );
        }
      } catch (err) {
    ...
    <div className="min-h-screen bg-snap-dark flex flex-col">
      ...
      <div className="bg-snap-darkMid border-b border-white/10 px-4 py-4 flex items-center justify-between sticky top-0 z-10">
          ...
          {peer ? (
            <>
              ...
              <div>
                ...
                <p
                  className={`text-xs ${isOnline ? "text-green-500" : "text-snap-white50"}`}
                >
        ...
        <div className="flex items-center gap-3">
          {callPending && (
            <span className="text-xs text-snap-yellow animate-pulse mr-2">
              Calling...
            </span>
          )}
          <button
