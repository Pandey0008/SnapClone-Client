...
const socket = io("http://localhost:3000", {
  auth: {
    userId: JSON.parse(localStorage.getItem("authData"))?.user?._id
  }
});
  ...
  */
  useEffect(() => {
    ...
    const fetchFriendsLocations = async () => {
      try {
        const res  = await fetch("http://localhost:3000/api/v1/location/friends", {
  ...
  */
  const toggleGhostMode = async () => {
    try {
      ...
      await fetch("http://localhost:3000/api/v1/users/location-visibility", {
        method: "PUT",
        headers: {
