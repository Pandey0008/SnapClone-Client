...
const StoryViewer = memo(({ stories, startIndex = 0, onClose }) => {
    ...
    <div className="fixed inset-0 bg-black z-50 flex flex-col">
      ...
      <div className="flex items-center justify-between px-4 pt-12 pb-4 bg-gradient-to-b from-black/80 to-transparent">
        <div className="flex items-center gap-3">
          <Avatar uri={currentStory?.user?.avatarUrl} name={currentStory?.user?.displayName} size={40} />
          <div>
            <p className="font-semibold">{currentStory?.user?.displayName}</p>
            <p className="text-xs text-snap-white50">{currentStory?.timestamp}</p>
          </div>
        </div>

        <button onClick={onClose} className="text-4xl leading-none text-white">
          <X size={28} />
        </button>
      </div>
